...
export const aiGenStatusEnum = pgEnum("ai_gen_status", ["pending", "processing", "completed", "failed"]);
export const analyticsEventEnum = pgEnum("analytics_event", [
  "page_view", "template_view", "invite_view", "invite_share",
  "rsvp_submit", "link_click", "qr_scan", "ad_impression", "ad_click",
]);
...
export const templates = pgTable("templates", {
  ...
  colors: jsonb("colors").notNull().$type<{
    ...
    accent: string; text: string; card: string;
  }>(),
...
export const templatePurchases = pgTable("template_purchases", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  templateId: varchar("template_id", { length: 100 }).notNull().references(() => templates.id),
  purchasedAt: timestamp("purchased_at").notNull().defaultNow(),
}, (t) => [
  uniqueIndex("user_template_idx").on(t.userId, t.templateId),
]);
...
export const events = pgTable("events", {
  ...
  index("events_invitation_idx").on(t.invitationId),
]);
...
export const creditTransactions = pgTable("credit_transactions", {
  ...
  index("credits_user_idx").on(t.userId),
]);
...
export const aiGenerations = pgTable("ai_generations", {
  ...
  result: jsonb("result").$type<{
    html?: string; css?: string; gradient?: string;
    colors?: Record<string, string>; suggestions?: string[];
  }>(),
  ...
  index("ai_gen_user_idx").on(t.userId),
]);
